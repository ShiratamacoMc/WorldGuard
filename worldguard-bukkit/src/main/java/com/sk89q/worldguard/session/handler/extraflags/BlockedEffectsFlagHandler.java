...
public class BlockedEffectsFlagHandler extends FlagValueChangeHandler<Set<PotionEffectType>>
{
	...
	{
		super(session, com.sk89q.worldguard.bukkit.protection.flags.BukkitExtraFlags.BLOCKED_EFFECTS);
	...
	{
		this.handleValue(player, player.getWorld(), set.queryValue(player, com.sk89q.worldguard.bukkit.protection.flags.BukkitExtraFlags.BLOCKED_EFFECTS));
	}
		...
		while (potionEffects_.hasNext())
		{
			...
			if (value == null || !value.contains(potionEffect.getKey()))
			{
				...
				if (removedEffect != null)
				{
					...
					if (timeLeft > 0)
					{
						bukkitPlayer.addPotionEffect(new PotionEffect(potionEffect.getKey(), timeLeft, removedEffect.getAmplifier(), removedEffect.isAmbient(), removedEffect.isParticles()));
					}
				}
